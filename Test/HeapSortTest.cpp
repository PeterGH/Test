...
void HeapSortTest::Init(void)
{
	...
	Add("D Sort 1", [&](){
		int I[] = { 3 };
		Test::HeapSort::Sort(I, 1, 3);
		ASSERT1(I[0] == 3);
	});

	Add("D Sort 2", [&](){
		int I[] = { 3, 1 };
		Test::HeapSort::Sort(I, 2, 3);
		ASSERT1(I[0] == 1);
		ASSERT1(I[1] == 3);
	});
	...
	Add("D Sort Random", [&](){
		for (int j = 0; j < 100; j++) {
			...
			Test::HeapSort::Sort(a.get(), length, dimension);
			std::make_heap(v.begin(), v.end());
			std::sort_heap(v.begin(), v.end());
	...
	Add("D Heapify", [&](){
		for (int j = 0; j < 100; j++) {
			...
			Test::HeapSort::Heapify(i1.get(), length, dimension);
			Test::HeapSort::ParallelHeapify(i2.get(), length, dimension);
